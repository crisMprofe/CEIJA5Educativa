...
const ListaEstudiantes = ({ onAccion, onClose, onVolver, soloParaEliminacion = false, refreshKey = 0, modalidadId }) => {
    ...
    const handleConfirmarEliminacion = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/eliminar-estudiantes/${estudianteAEliminar.dni}`, {
                method: 'PATCH',
                headers: {
                            ...
                            <tbody>
                                {estudiantes.map((estudiante, index) => (
                                    <tr key={`${estudiante.dni}-${estudiante.id}-${index}`}>
                                        <td>{estudiante.id}</td><td>{estudiante.dni}</td><td>{`${estudiante.nombre} ${estudiante.apellido}`}</td><td>{estudiante.email || 'No registrado'}</td><td>{estudiante.modalidad}</td><td>{estudiante.cursoPlan || 'Sin asignar'}</td><td><span className={`estado estado-${estudiante.estadoInscripcion?.toLowerCase().replace(/\s+/g, '-')}`}>{estudiante.estadoInscripcion || 'Sin estado'}</span></td><td>{formatearFecha(estudiante.fechaInscripcion)}</td><td>{formatearFecha(estudiante.fechaNacimiento)}</td><td><div className="acciones-grupo">{soloParaEliminacion ? (<button className="btn-accion btn-eliminar" onClick={() => handleAccion('Ver', estudiante)} title="Seleccionar para eliminar de la base de datos">⚠️ Eliminar</button>) : (<><button className="btn-accion btn-modificar" onClick={() => handleAccion('Modificar', estudiante)} title="Modificar estudiante">✏️</button><button className="btn-accion btn-eliminar" onClick={() => handleEliminarClick(estudiante)} title="Desactivar estudiante (eliminación lógica)">❌</button><button className="btn-accion btn-ver" onClick={() => handleAccion('Ver', estudiante)} title="Ver detalles">👁️</button></>)}
                                            </div></td>
                                    </tr>
                                ))}
                            </tbody>
